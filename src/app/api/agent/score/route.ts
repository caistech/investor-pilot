...
import { runScoreForPartner } from '@/lib/agent/pipeline';
...
export async function POST(request: Request) {
  ...
  const result = await runScoreForPartner(product, candidate);

  if (result.success && result.data.scored_partner) {
    ...
    const partner = result.data.scored_partner as Record<string, unknown>;
