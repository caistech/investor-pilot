/**
 * Synthesize a Product profile from 8 conversational interview answers.
 *
 ...
 * synthesizer turns them into the 13 structured product fields without
...
const SYSTEM_PROMPT = `You convert 8 interview answers about an operator's product into a structured product profile. The operator answered conversational questions; your job is to extract the structured fields the InvestorPilot prospecting system needs.
...
function buildUserMessage(answers: InterviewAnswer[]): string {
  const formatted = answers
    ...
    .join('\n\n');
  return `Synthesize a product profile from the following 8 interview answers. Return the JSON shape only.\n\n${formatted}`;
}
