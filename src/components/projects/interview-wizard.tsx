...
/**
 * Multi-step Project Interview wizard (funding side).
 *
 * Mirrors src/components/products/interview-wizard.tsx but uses the
 * funding-side question set and synthesis endpoint. Same UX: one question
 * per screen, progress bar with per-step status, Back/Next + "Synthesize
 * now" shortcut once required questions are answered, on success hand
 * the structured profile back to the parent for review/edit/save.
 */

import { useMemo, useState } from 'react';
import { ArrowLeft, ArrowRight, Sparkles, X, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-react';
import { PROJECT_INTERVIEW_QUESTIONS } from '@/lib/projects/interview-questions';
...
interface Props {
  ...
  onSynthesized: (profile: SynthesizedProjectProfile) => void;
  /** Called when operator cancels or closes mid-flow. */
  onCancel: () => void;
}

export default function ProjectInterviewWizard({ onSynthesized, onCancel }: Props) {
  const [stepIndex, setStepIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const questions = PROJECT_INTERVIEW_QUESTIONS;
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await fetch('/api/projects/synthesize-from-interview', {
        method: 'POST',
      ...
      onSynthesized(json.profile as SynthesizedProjectProfile);
    } catch (err) {
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-lg font-semibold truncate">Project Interview</h2>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {!isLast ? (
            ...
            <button
              ...
              title={canSubmit ? 'Generate the structured project profile from these answers' : `${requiredTotal - requiredAnswered} required question(s) still empty`}
            >
